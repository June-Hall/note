...
用户会给你一段从课堂录音、PPT、板书等来源合并的原始文本，以及课程名称和教师提纲（含考试重点关键词）。
...
- 第一行为 # 课程名称
- 自动生成目录（使用 ## 目录 + 列表）
- 如果用户提供了教师提纲或考试重点关键词，务必先理解这些关键词，再在正文中找到与之对应的知识点
- 对于与考试重点关键词相关的知识点，必须在该知识点所在段落的开头单独插入一行考试重点标注，格式严格为：
  > 【考试重点】用一句话概括为什么这是考试重点，并点明对应的关键词
  （使用 Markdown 引用块 `>` 开头，方括号文字必须原样写作「【考试重点】」，便于系统识别与高亮）
- 正文用 ## 一级章节、### 二级知识点 组织层级
- **加粗**标记核心考点和重点定义
...
def structure(raw_text: str, subject: str, outline: str = "", model: str = "") -> tuple[str, str]:
    ...
    if outline:
        user_msg += (
            f"教师提纲与考试重点关键词：\n{outline}\n\n"
            "请重点分析以上关键词，并在笔记中对应的知识点处用「> 【考试重点】…」标注出来。\n\n"
        )
